...
# In order to make this code faster:
...
def compute_cov_a(a, classname, layer_info, fast_cnn):
    ...
    return a.t().__matmul__(a / batch_size)
...
def compute_cov_g(g, classname, layer_info, fast_cnn):
    ...
    g_ = g * batch_size
    return g_.t().__matmul__(g_ / g.size(0))
...
class KFACOptimizer(optim.Optimizer):
    ...
    def step(self):
        ...
        for i, m in enumerate(self.modules):
            ...
            v1 = self.Q_g[m].t().__matmul__(p_grad_mat.__matmul__(self.Q_a[m]))
            v2 = v1 / (
                self.d_g[m].unsqueeze(1) * self.d_a[m].unsqueeze(0) + la)
            v = self.Q_g[m].__matmul__(v2.__matmul__(self.Q_a[m].t()))
